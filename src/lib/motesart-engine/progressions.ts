...
import { MotesartChord, DetectedProgression } from './types';

/** Known progression patterns with their names */
const KNOWN_PROGRESSIONS: { pattern: string; name: string; description: string }[] = [
 ...
 */
function getRootForPattern(chord: MotesartChord): string {
 ...
 */
function getBareRoot(chord: MotesartChord): string {
 ...
 */
export function detectProgressions(chords: MotesartChord[]): DetectedProgression[] {
  if (chords.length < 2) return [];

  const detected: DetectedProgression[] = [];
      ...
      if (windowStr === prog.pattern.replace(/m/g, '')) {
        ...
        if (qualityMatch) {
          ...
          const alreadyDetected = detected.some(
            d => d.pattern === prog.pattern && d.startMeasure === i
          );
          if (!alreadyDetected) {
            detected.push({
