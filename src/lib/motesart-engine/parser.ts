...
import { getKeyByTonic, detectKeyFromChords, NOTE_TO_PITCH } from './keys';
import { chordToMotesart, parseChordSymbol } from './chords';
import { detectProgressions } from './progressions';
...
export function detectInputFormat(text: string): InputFormat {
  ...
  for (const line of lines) {
    if (isChordLine(line)) hasChordLines = true;
    else if (!isSectionHeader(line) && line.trim().length > 0) hasLyricLines = true;
  }

  if (hasChordLines && hasLyricLines) return 'chords-over-lyrics';
  if (hasChordLines) return 'plain-sequence';
 ...
 */
function parseInlineChords(text: string): ChordChartLine[] {
  const lines = text.split('\n');
 ...
 */
export function parseChordChart(text: string): {
    ...
    for (const line of lines) {
      if (line.chords) {
        for (const c of line.chords) {
          ...
          if (parsed) allChordRoots.push(parsed.root);
        }
      }
    }
    return {
  ...
  let currentSection: ChordChartSection = { name: 'Intro', lines: [] };
 ...
 */
export function convertChordChart(
  text: string,
  options?: {
    key?: string;
    timeSignature?: string;
  ...
  if (options?.key) {
    key = getKeyByTonic(options.key) || detectKeyFromChords(chordRoots);
  } else if (metadata.key) {
    key = getKeyByTonic(metadata.key) || detectKeyFromChords(chordRoots);
  } else {
    key = detectKeyFromChords(chordRoots);
  }

  // Step 3: Convert all chords in every section
  const allMotesartChords: import('./types').MotesartChord[] = [];

  for (const section of sections) {
    for (const line of section.lines) {
      if (line.chords && line.chords.length > 0) {
        line.motesartChords = line.chords
          .map(c => chordToMotesart(c, key))
          .filter((c): c is import('./types').MotesartChord => c !== null);
  ...
  const detectedProgressions = detectProgressions(allMotesartChords);
