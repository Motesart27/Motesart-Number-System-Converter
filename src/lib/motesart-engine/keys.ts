...
import { KeySignature, PitchClass } from './types';
...
export const NOTE_TO_PITCH: Record<string, PitchClass> = {
  'C': 0, 'C#': 1, 'Db': 1,
  'D': 2, 'D#': 3, 'Eb': 3,
  'E': 4, 'Fb': 4, 'E#': 5,
  'F': 5, 'F#': 6, 'Gb': 6,
 ...
 */
export function getKeyByTonic(tonic: string): KeySignature | undefined {
  // Direct match
  const direct = KEY_SIGNATURES.find(k => k.tonic === tonic);
  if (direct) return direct;

  // Enharmonic match: find by pitch class
  const pitch = NOTE_TO_PITCH[tonic];
  if (pitch !== undefined) {
    return KEY_SIGNATURES.find(k => k.tonicPitch === pitch);
  }
  return undefined;
}
 ...
 */
export function pitchToMotesartNumber(pitch: PitchClass, key: KeySignature): string {
  ...
  const halfNumberMap: Record<number, string> = {
    1: '1½',
    3: '2½',
    6: '4½',
    8: '5½',
 ...
 */
export function isDiatonic(pitch: PitchClass, key: KeySignature): boolean {
 ...
 */
export const DIATONIC_QUALITIES: Record<number, 'major' | 'minor' | 'dim'> = {
  1: 'major',
  2: 'minor',
  3: 'minor',
  4: 'major',
 ...
 */
export function detectKeyFromChords(chordRoots: string[]): KeySignature {
  let bestKey = KEY_SIGNATURES[0]; // Default to C Major
  let bestScore = -1;
