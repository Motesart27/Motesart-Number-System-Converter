...
exports.convertChordToMotesart = convertChordToMotesart;
exports.chordToMotesart = chordToMotesart;
...
function parseChordSymbol(chordStr) {
    ...
    const rootPitch = keys_1.NOTE_TO_PITCH[rootName];
    ...
    const bassPitch = bassName ? keys_1.NOTE_TO_PITCH[bassName] : undefined;
    return {
        original: trimmed,
        root: rootName,
        rootPitch: rootPitch,
 ...
 * §4c: 7→⁷, 9→⁹, 11→¹¹, 13→¹³
 */
function formatExtensions(extensions, isMaj7) {
 ...
 */
function convertChordToMotesart(parsed, key) {
    // Step 1: Convert root note to Motesart number
    const rootNumber = (0, keys_1.pitchToMotesartNumber)(parsed.rootPitch, key);
    ...
    if (parsed.isHalfDim) {
        ...
        const rootIsDiatonic = (0, keys_1.isDiatonic)(parsed.rootPitch, key);
        if (rootIsDiatonic) {
            // Find which scale degree this root is
            const semitones = ((parsed.rootPitch - key.tonicPitch) % 12 + 12) % 12;
            const degreeIndex = keys_1.MAJOR_SCALE_INTERVALS.indexOf(semitones);
            if (degreeIndex !== -1) {
                const degree = degreeIndex + 1;
                const naturalQuality = keys_1.DIATONIC_QUALITIES[degree];
    ...
    if (parsed.bass && parsed.bassPitch !== undefined) {
        bassNumber = (0, keys_1.pitchToMotesartNumber)(parsed.bassPitch, key);
        isInversion = true;
    }
 ...
 */
function chordToMotesart(chordStr, key) {
